...
$( document ).on ( "cloudflare.firewall.security_level.initialize", function ( event, data ) {
	$(data.section).find ("[name='value']").val ( data.response.result.value )
	$(data.section).removeClass ("loading")
});

$( document ).on ( "cloudflare.firewall.security_level.update", function ( event, data ) {
