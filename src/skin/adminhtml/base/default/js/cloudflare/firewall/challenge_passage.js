...
$( document ).on ( "cloudflare.firewall.challenge_passage.initialize", function ( event, data ) {
	$(data.section).find ("[name='value']").val ( data.response.result.value )
	$(data.section).removeClass ("loading")
});

$( document ).on ( "cloudflare.firewall.challenge_passage.update", function ( event, data ) {
