...
$( document ).on ( "cloudflare.crypto.minimum_tls_version.initialize", function ( event, data ) {
	$(data.section).find ("[name='value']").val ( data.response.payload.value )
	$(data.section).removeClass ("loading")
});

$( document ).on ( "cloudflare.crypto.minimum_tls_version.change", function ( event, data ) {
