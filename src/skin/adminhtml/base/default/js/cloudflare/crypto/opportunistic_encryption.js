...
$( document ).on ( "cloudflare.crypto.opportunistic_encryption.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.crypto.opportunistic_encryption.toggle", function ( event, data ) {
