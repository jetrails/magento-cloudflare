...
$( document ).on ( "cloudflare.crypto.automatic_https_rewrites.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.crypto.automatic_https_rewrites.toggle", function ( event, data ) {
