...
$( document ).on ( "cloudflare.crypto.always_use_https.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.crypto.always_use_https.toggle", function ( event, data ) {
