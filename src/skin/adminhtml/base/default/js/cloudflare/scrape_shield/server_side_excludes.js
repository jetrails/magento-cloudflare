...
$( document ).on ( "cloudflare.scrape_shield.server_side_excludes.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.scrape_shield.server_side_excludes.toggle", function ( event, data ) {
