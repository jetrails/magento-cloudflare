...
$( document ).on ( "cloudflare.scrape_shield.email_address_obfuscation.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.scrape_shield.email_address_obfuscation.toggle", function ( event, data ) {
