...
$( document ).on ( "cloudflare.scrape_shield.hotlink_protection.initialize", function ( event, data ) {
	var value = data.response.payload.value == "on"
	$( data.section ).find ("[name='mode']").prop ( "checked", value )
});

$( document ).on ( "cloudflare.scrape_shield.hotlink_protection.toggle", function ( event, data ) {
